...
/*
 * AArch64: the 68k N, Z, C and V flags live at the ARM NZCV bit positions
 * (31..28) so the JIT can copy the host condition register in and out
 * directly. nzcv is the JIT's name for the field and cznv the interpreter's;
 * the anonymous union makes them the same storage. X is kept as 0/1 in bit 0.
 */
struct flag_struct {
    union {
        uae_u64 nzcv;
...
#define SET_NFLG(y)		(regflags.nzcv = (regflags.nzcv & ~(uae_u64)FLAGVAL_N) | (((uae_u64)(y) & 1) << FLAGBIT_N))
#define SET_ZFLG(y)		(regflags.nzcv = (regflags.nzcv & ~(uae_u64)FLAGVAL_Z) | (((uae_u64)(y) & 1) << FLAGBIT_Z))
#define SET_CFLG(y)		(regflags.nzcv = (regflags.nzcv & ~(uae_u64)FLAGVAL_C) | (((uae_u64)(y) & 1) << FLAGBIT_C))
#define SET_VFLG(y)		(regflags.nzcv = (regflags.nzcv & ~(uae_u64)FLAGVAL_V) | (((uae_u64)(y) & 1) << FLAGBIT_V))
