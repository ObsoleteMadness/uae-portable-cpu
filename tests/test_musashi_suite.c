...
int main(int argc, char* argv[]) {
    ...
    if (argc < 2) {
        printf("Usage: %s <binary.bin | test_dir> [cpu_type (68000|68040)]\n", argv[0]);
        return 1;
    }
    ...
    if (!dir) {
        /* Single file mode */
        printf("[*] Running single test: %s (CPU: %s)...\n", target, (cpu_type == M68K_CPU_TYPE_68040) ? "68040" : "68000");
    ...
    printf("=== Running Musashi Test Suite on %s ===\n\n", target);
    struct dirent *entry;
    int total = 0, passed = 0, failed = 0;

    char bin_paths[256][512];
    int bin_count = 0;

    while ((entry = readdir(dir)) != NULL) {
        char *dot = strrchr(entry->d_name, '.');
        if (dot && strcmp(dot, ".bin") == 0) {
            snprintf(bin_paths[bin_count++], sizeof(bin_paths[0]), "%s/%s", target, entry->d_name);
        }
    }
    closedir(dir);
    ...
    for (int i = 0; i < bin_count; i++) {
        const char *name = strrchr(bin_paths[i], '/');
        name = name ? name + 1 : bin_paths[i];
        printf("[%2d/%2d] %-22s ... ", i + 1, bin_count, name);
        fflush(stdout);

        int res = run_single_test(bin_paths[i], cpu_type, 0);
        total++;
        if (res == 0) {
    ...
    printf("\n=== Musashi Test Suite Summary ===\n");
    printf("Total:     %d\n", total);
    printf("Passed:    %d\n", passed);
    printf("Failed:    %d\n", failed);
    printf("Pass Rate: %.1f%%\n", (double)passed * 100.0 / (total ? total : 1));

    if (passed >= total * 0.85 && passed > 0) {
        printf("\n>>> MUSASHI SUITE VERIFICATION SUCCESS (Pass Rate >= 85%%) <<<\n\n");
        return 0;
    }
    return (failed > 0) ? 1 : 0;
}
