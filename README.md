...
- **Direct memory access** (`jit_direct_memory`). Translated code reads and writes RAM inline as `base + address` instead of calling a handler. This is how WinUAE and Amiberry run fast, and it is several times faster than handler calls. It applies to regions mapped with `UAE_MEM_JIT_DIRECT` whose host pointer is `base + start`. Each block is profiled in the interpreter before it is compiled: accesses that touched only such regions are inlined, all others keep the handler call. An instruction that later reaches a different address still goes to `base + address`, so the window must cover every address translated code can reach. In practice that means one 4 GB reservation with RAM, ROM and video memory committed at their guest addresses and the rest left inaccessible. On x86-64 (every OS) and ARM64 (Linux, macOS and Windows), a fault in an inaccessible part of the window is recovered: the access completes through the region's handler and the block is profiled again. A handler reached this way cannot raise a bus error. On ARM64, recovery covers the integer loads and stores; a translated FPU or MOVE16 access that faults still reaches the host's handler.
  - **ROM must be read-only on the host.** A store that profiling saw hit ROM keeps the handler call, which drops it, but a store inlined because it first hit RAM writes `base + address` wherever it later lands, ROM included. Guest operating systems do this (Mac OS writes to driver headers that live in ROM). Commit ROM pages read-only: the store then faults and recovery drops it, as the handler would.
...
- **JIT FPU** (`jit_fpu`). Translates FPU instructions too, as WinUAE's JIT FPU does. It needs the host-double FPU backend (`fpu_softfloat = false`), since translated FPU code works on host doubles; results have double, not 80-bit extended, precision. It works with or without `jit_direct_memory`: FPU operands in memory follow the same inline-or-handler choice as integer accesses, and double and extended values are assembled from 32-bit handler accesses when the handlers are used.
...
- **Self-modifying code.** Call `uae_cpu_invalidate_code()` after the host writes guest code. Only the blocks whose source overlaps the range are disturbed, and they are checksummed before reuse, so unchanged code is kept; pass a size of `UINT32_MAX` when the write cannot be bounded. Guest code that writes code and flushes its own instruction cache (68040+ `CINVA`/`CPUSHA`, 68020/030 `CACR`) is honoured the same way; set `jit_ignore_guest_cache_flush` if the host reports every code write itself and the guest flushes often enough for it to cost. On ARM64, blocks in `UAE_MEM_ROM` regions are not checksummed and survive guest cache flushes; a host that rewrites ROM code still reports it with `uae_cpu_invalidate_code()`.

Verification: the m68k-rs fixtures give the same per-fixture results with the interpreter, the JIT and the JIT with direct memory access on the same memory map. `host_hooks_jit` and `host_hooks_jit_direct` check translated byte/word/long access against a C reference. The direct variant also checks that devices and regions outside the window keep their handlers, and exercises fault recovery on x86-64 and ARM64: a translated read moved onto a device, and translated byte, word and long stores moved from RAM onto read-only ROM. CI builds and tests the JIT with GCC and Clang on Linux and macOS, and with MSVC (x64 and ARM64) and MinGW-w64 on Windows. `uae_cpu_bench` measures throughput; see [Benchmarks](#benchmarks).

Limitations: direct memory access needs a window covering the guest address space and read-only ROM pages; the JIT FPU computes in double precision; one CPU per process.
